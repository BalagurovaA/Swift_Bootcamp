# Project 02 — Swift_Bootcamp

В проекте реализованы консольные проекты, которые направлены на знакомство ООП/процедурным/мультипарадигмальным подходом в Swift. Код придерживается функциональной парадигмы. 
## Содержание
...
- Реализуй поле description в структуре Dog таким образом, чтобы формировалась следующая строка: «Dog name = [имя_питомца], age = [возраст_питомца]».
- Создай структуру Cat, которая реализует протокол Animal.
- Реализуй поле description в структуре Cat таким образом, чтобы формировалась следующая строка: «Cat name = [имя_питомца], age = [возраст_питомца]».
...
- Реализуй метод toString() в классе Dog, чтобы формировалась следующая строка: «Dog name = [имя_питомца], age = [возраст_питомца], mass = [масса_питомца]».
...
- Создай класс Dog, который реализует протокол Animal.
- Реализуй конструктор для класса Dog, который принимает два параметра: строка имени питомца, целочисленный возраст и присваивает их соответствующим полям.
- Реализуй метод toString() в классе Dog, чтобы формировалась следующая строка: «Dog name = [имя_питомца], age = [возраст_питомца]».
- Создай класс Cat, который реализует протокол Animal.
- Реализуй конструктор для класса Cat, который принимает два параметра: строка имени питомца, целочисленный возраст и присваивает их соответствующим полям.
- Реализуй метод toString() в классе Cat, чтобы формировалась следующая строка: «Cat name = [имя_питомца], age = [возраст_питомца]».
- Программа считывает количество питомцев.
- Программа считывает тип вводимого питомца: dog/cat.
- Каждый питомец добавляется в общий список pets.
- Если ввели неправильный тип питомца, то программа выводит: «Incorrect input. Unsupported pet type» и переходит к следующему вводу.
...
- Реализуй конструктор для класса Dog, который принимает два параметра: строка имени питомца, целочисленный возраст и присваивает их соответствующим полям.
- Реализуй метод toString() в классе Dog, чтобы формировалась следующая строка: «Dog name = [имя_питомца], age = [возраст_питомца]».
- Реализуй метод goToWalk() в классе Dog следующим образом: метод вычисляет время прогулки в секундах, вызывает TimeUnit.SECONDS.sleep() для вычисленного времени, возвращает вычисленное время.
- Создай класс Cat, который реализует протокол Animal.
- Реализуй конструктор для класса Cat, который принимает два параметра: строка имени питомца, целочисленный возраст и присваивает их соответствующим полям.
- Реализуй метод toString() в классе Cat, чтобы формировалась следующая строка: «Cat name = [имя_питомца], age = [возраст_питомца]».
...
- Реализуй конструктор для класса Cat, который принимает два параметра: строка имени питомца, целочисленный возраст и присваивает их соответствующим полям.
- Реализуй метод toString() в классе Cat, чтобы формировалась следующая строка: «Cat name = [имя_питомца], age = [возраст_питомца]».
- Создай протокол BaseIterator<T>
- Объяви в протоколе BaseIterator<T> метод next(), который возвращает элемент типа T
- Объяви в протоколе BaseIterator<T> метод hasNext(), который возвращает boolean
- Объяви в протоколе BaseIterator<T> метод reset(), который ничего не возвращает
- Создай класс AnimalIterator<Animal>, который реализует интерфейс BaseIterator
